...
try:
	...
	def mapView_regionDidChangeAnimated_imp(
			self, cmd, mk_mapview, animated
		):
	...
	imp = IMPTYPE(
		mapView_regionDidChangeAnimated_imp
	)
	...
	class_ptr = c.objc_allocateClassPair(
		NSObject.ptr, 
		(
			py23('OMMapViewDelegate')
		), 
		0
	)
	selector = sel(
		py23('mapView:regionDidChangeAnimated:')
	)
	c.class_addMethod(
		class_ptr,
		selector,
	...
	def mapView_viewForAnnotation_imp(
		...
		return mv
		#).initWithAnnotation_reuseIdentifier_(annotation, identifier)
	...
	# This is a little ugly, but we need to make sure that `imp` isn't garbage-collected:
	ui._retain_me_mapview_delegate_imp2 = imp2
	sel2 = sel(
		py23('mapView:viewForAnnotation:')
	)
...
class MapView (ui.View):
	...
	def add_pin(self, lat, lon, title, subtitle=None, select=False):
		...
		#mkmarkview = ObjCClass('MKMarkerAnnotationView')
		#mk = mkmarkview.alloc().init().autorelease()
...
def main():
	...
	if loc:
		...
		for name,d in stations.items():
			b,s=bikes.get_bikes_and_spaces(
				d['id']
			)
